...
set usage="Usage: SIPHONS_win <netname>"
...
"%SHARED_BIN_DIR%\siphons.exe" "%NETNAME%"

if ERRORLEVEL 1 (
  echo ERROR while Siphons execution.
  GOTO END_SCRIPT
)  
...
if EXIST "%NET%.siphons" (
move "%NET%.siphons" "%NET%.dir" >NUL 2>&1
)
if EXIST "%NET%.INV" (
